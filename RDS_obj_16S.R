...
## Description: 
### - Creation of 16S ps_object from ASV, taxonomy and sample metadata matrixes.
...
#Load ASV matrix
ASV_mat <- read.csv("16_ALL_mod_asv_otu.csv", row.names=1)
#Fortmatting ASV matrix
ASV_mat <- ASV_mat %>% 
  rownames_to_column(var ="ASV")

names(ASV_mat) <- gsub(x = names(ASV_mat), pattern = "_16S_S[0-9]+_L00[0-9]+_R1_trimmed_val_1.fq.gz", replacement = "_16S") 
...
tax_mat <- read.csv("16_ALL_mod_asv_tax.csv", row.names=1)
#Fortmatting taxonomy matrix
tax_mat <- tax_mat %>% 
  rownames_to_column(var = "ASV")

#Load sample metadata table
samples_df  <- read.csv("sample_metadata_16S.csv",sep = ";")
#Fortmatting taxonomy matrix
...
dat.aglo_c <- subset_taxa(ps0, Class!="Chloroplast")
dat.aglo_c
#removing mitochondria 
dat.aglo_m <- subset_taxa(dat.aglo_c, Family!="mitochondria")
dat.aglo_m

#removing mitochondria 
dat.aglo_M <- subset_taxa(dat.aglo_m, Family!="Mitochondria")
dat.aglo_M
#removing Archaea beacuse primers should not target this kingdom.
dat.aglo_a <- subset_taxa(dat.aglo_M, Kingdom!="Archaea")
dat.aglo_a

# Keep taxa with 3 or more ounts in at least >2% of the samples.
ps_filter = filter_taxa(dat.aglo_a, function(x) sum(x >=3) > (0.02*length(x)), TRUE) 
ps_filter
...
#Create RDS object
saveRDS(ps_filter, "ps_filter_16S.rds")
saveRDS(ps_filtered_norm, "ps_filtered_norm_16S.rds")
